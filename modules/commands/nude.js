module.exports.config = {
  ...
  description: "Random ảnh nhưng mất 1 tỷ",
  commandCategory: "Ảnh",
  usages: "/nude",
  cooldowns: 2,
  dependencies: {
...
module.exports.run = async({ api, event, Currencies }) => {
  ...
  if (money < cost) {
    return api.sendMessage(
      `⚠️ Bạn cần ${cost.toLocaleString()} đô để dùng lệnh này 😿\n💰 Số dư hiện tại: ${money.toLocaleString()} đô`,
      event.threadID,
      event.messageID
    );
  } else {
    ...
    }, event.threadID, () => fs.unlinkSync(__dirname + "/cache/nude.jpg"));

    return request(encodeURI(link[Math.floor(Math.random() * link.length)]))
      .pipe(fs.createWriteStream(__dirname + "/cache/nude.jpg"))
      .on("close", () => callback());
  }
};
